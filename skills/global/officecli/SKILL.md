...
name: officecli
description: >-
  Create, read, edit, proofread, and render Office documents (.docx, .xlsx,
  .pptx) via the officecli binary — one self-contained CLI, no Office/LibreOffice
  needed. Use whenever a Word / Excel / PowerPoint file is the primary input or
  output: create from scratch; inspect / outline / extract; check formatting or
  find issues; add or edit text, tables, charts, images; build pivot tables;
  evaluate formulas (350+ functions, auto-calculated on write); merge templates;
  or render to HTML / PNG / SVG / PDF for visual verification. Replaces the former
  docx / xlsx / pptx Python skills. NOT for PDF-native editing (use `pdf`) or for
  converting files to Markdown (use `markitdown`).
allowed-tools: Read, Write, Bash
metadata:
  category: document
  tags: [office, docx, xlsx, pptx, cli, formulas, pivot, rendering]
  requires: >-
    officecli binary on PATH — install/upgrade with `npm install -g
    @officecli/officecli` (managed by this repo; see INSTALL.md).
  source: >-
    Vendored from `iOfficeAI/OfficeCLI` (Apache-2.0). Body is the upstream
    SKILL.md; only the Install section is adapted to this repo's npm-based setup.
...
Managed by this repo via **npm**. On a synced machine `officecli` is already on
PATH — check with `officecli --version`.

```bash
npm install -g @officecli/officecli      # install / upgrade (the method this repo uses)
officecli --version                       # verify
```

The official installers (`curl … install.sh | bash`, `irm … install.ps1 | iex`,
Homebrew / Scoop) also work, but their installer auto-drops a copy of this
SKILL.md into every detected agent directory. This repo owns the skill file
instead (single source of truth), so prefer npm here. If `officecli` is still not
found right after install, open a new terminal.
