...
`{agent}` expands to one of `claude, cursor, codex, opencode, pi`.
Pi is the exception to the `.{agent}/skills` shorthand: global `~/.pi/agent/skills/`,
...
| --- | --- | --- | --- |
| `skills/global/*/` | `{user_home}/.{agent}/skills/` for each agent | yes | 38 skills, identical across all 5 agents (incl. `officecli`, `handoff`, `action-first`) |
...
| `agents/pi/AGENTS.md` | `~/.pi/agent/AGENTS.md` | yes (agent=pi) |

> **Pi has no MCP config and no hooks row** — both are absent from Pi upstream by design.
> Pi's global skills go to `~/.pi/agent/skills/`, but its *project-pack* skills go to
> `<project>/.pi/skills/` (no `agent` segment) — the only agent where the two differ.
> Never touch `~/.pi/agent/auth.json`; sync mirrors only the `skills/` subdirectory.

## MCP server configs

All four templates use env-var references only. No plaintext secrets.
(Four, not five — Pi has no MCP support.)
...
`agents/opencode/AGENTS.md`, `agents/pi/AGENTS.md`, `agents/cursor/rules/global-rules.md`,
