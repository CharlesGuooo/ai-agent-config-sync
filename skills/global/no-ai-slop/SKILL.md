...
name: no-ai-slop
description: >-
  Edit a draft into sharper, more human writing while preserving the writer's personal voice,
  or detect AI-slop patterns without rewriting. Use when the user hands over a draft and wants
  it clearer, more direct, more opinionated, or less AI-sounding, or asks whether a piece reads
  as AI. Triggers: "去 AI 味", "这段是不是很 AI", "改得像人写的", "润色一下这段",
  "audit this for slop", "does this read as AI". Operates on a draft the user supplies — it does
  not police the agent's own prose. Distinct from action-first, which shortens the agent's own replies.
allowed-tools: Read, Write, Edit
license: MIT
metadata:
  category: process
  tags: [writing, editing, style, voice, slop]
  source: >-
    Body and eval.md vendored verbatim from github.com/petergyang/no-ai-slop (MIT, 4.2k★,
    scanned clean: 0 findings, 0 URLs, no scripts). Frontmatter adapted for this harness —
    allowed-tools restricted to Read/Write/Edit, Chinese triggers added, scope boundary stated.
---
